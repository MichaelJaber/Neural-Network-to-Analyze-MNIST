...
class Network(object):
    def __init__(self, layers):
        ...
        for x , y in list(zip(layers[:-1], layers[1:])):
    ...
    def update_network(self, mini_batch, learning_rate):
        ...
        for x , y in mini_batch:
            ...
            for dgrad_b, grad_b in list(zip(delta_grad_wrt_b, grad_wrt_b)):
                grad_wrt_b = [dgrad_b + grad_b]
            for dgrad_w, grad_w in list(zip(delta_grad_wrt_w, grad_wrt_w)):
                grad_wrt_w = [dgrad_w + grad_w]
        
        for b , grad_wrt_b in list(zip(self.biases, grad_wrt_b)):
            self.biases = [ b - ( learning_rate / len(mini_batch)) * grad_wrt_b]
        for w , grad_wrt_w in list(zip(self.weights, grad_wrt_w)):
